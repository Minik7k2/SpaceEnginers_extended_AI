// AiEnabledApi.cs — KLIENT API moda AiEnabled (Workshop 2596208372), skopiowany do naszego
// moda tak samo jak MESApi.cs. Oryginał pochodzi z Modular Encounters Systems
// (Data/Scripts/ModularEncountersSystems/API/AiEnabledApi.cs) — to autorska ścieżka
// integracji: MES sam wozi ten plik i przez niego stawia boty. Dzięki temu mamy pewność,
// że kod przechodzi whitelistę ModAPI, bo dokładnie ten plik działa dziś w grze w MES.
//
// Zmieniona jest WYŁĄCZNIE przestrzeń nazw (ModularEncountersSystems.API -> ZyweFrakcje),
// żeby dało się porównać plik z oryginałem przy aktualizacji MES. Nic poza tym nie ruszać:
// każda zmiana utrudni diff, a kontrakt (kanał wiadomości, sygnatury delegatów) musi się
// zgadzać z tym, czego oczekuje AiEnabled.
//
// Kluczowe dla nas metody: SpawnBotQueuedWithId (bot z rolą, właścicielem i kolorem),
// SetBotPatrol / SetBotTarget / SetBotGoto (rozkazy z brainu), TrySeatBot (obsada stanowisk),
// GetInteriorNodes (realne węzły wewnątrz kadłuba — bez zgadywania, gdzie postawić bota),
// CheckBotRelationTo i RemoveBot.
